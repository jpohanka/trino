        ...
        implements ConnectorMetadata
{
    ...
    public ConnectorTableMetadata getTableMetadata(ConnectorSession session, ConnectorTableHandle table)
    {
        ...
        if (pinotTableHandle.getQuery().isPresent()) {
            ...
            for (PinotColumnHandle pinotColumnHandle : dynamicTable.getSelections()) {
            ...
            List<List<ColumnHandle>> groupingSets)
    {
        ...
        if (tableHandle.getQuery().isPresent() &&
                (!tableHandle.getQuery().get().getAggregateColumns().isEmpty() ||
                        tableHandle.getQuery().get().isAggregateInSelectList() ||
        ...
        if (tableHandle.getQuery().isPresent()) {
            ...
            Map<String, PinotColumnHandle> selectionsMap = tableHandle.getQuery().get().getSelections().stream()
                    .collect(toImmutableMap(PinotColumnHandle::getColumnName, identity()));
            groupingColumns = groupingColumns.stream()
                    .map(groupIngColumn -> selectionsMap.getOrDefault(groupIngColumn.getColumnName(), groupIngColumn))
                    .collect(toImmutableList());
            ...
            aggregationColumns = aggregationColumns.stream()
                    .map(aggregateExpression -> resolveAggregateExpressionWithAlias(aggregateExpression, selectionsMap))
                    .collect(toImmutableList());
    ...
    private static PinotColumnHandle resolveAggregateExpressionWithAlias(PinotColumnHandle aggregateColumn, Map<String, PinotColumnHandle> selectionsMap)
    {
        ...
        PinotColumnHandle selection = selectionsMap.get(aggregateColumn.getPushedDownAggregateFunctionArgument().get());
